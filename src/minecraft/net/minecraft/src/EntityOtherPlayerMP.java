...
public class EntityOtherPlayerMP extends EntityPlayer
{
    ...
    public EntityOtherPlayerMP(World par1World, String par2Str)
    {
        ...
        if (!mod_SkinFix.getInstance().getFetchedUUID().contains(this.username)) {
			...
			try {
				...
				if (result.length() != 0) {
					...
					mod_SkinFix.getInstance().getUUIDBank().put(this.username, kept);
				}
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
			mod_SkinFix.getInstance().getFetchedUUID().add(this.username);
		}
        

        if (par2Str != null && par2Str.length() > 0)
        {
        	this.skinUrl = "https://crafatar.com/skins/" + mod_SkinFix.getInstance().getUUIDBank().get(this.username) + ".png";
        }
