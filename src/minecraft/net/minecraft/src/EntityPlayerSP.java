...
public class EntityPlayerSP extends EntityPlayer {
	...
	public EntityPlayerSP(Minecraft par1Minecraft, World par2World, Session par3Session, int par4) {
		...
		this.dimension = par4;
		if (!mod_SkinFix.getInstance().getFetchedUUID().contains(par3Session.username)) {
			StringBuilder result = new StringBuilder();
			try {
				URL url = new URL("https://api.mojang.com/users/profiles/minecraft/" + par3Session.username);
				...
				if (result.length() != 0) {
					String s1 = result.toString().substring(result.indexOf(":\"") + 2);
					String kept = s1.substring(0, s1.indexOf("\",\""));
					mod_SkinFix.getInstance().getUUIDBank().put(par3Session.username, kept);
				}
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
			mod_SkinFix.getInstance().getFetchedUUID().add(par3Session.username);
		}

		if (par3Session != null && par3Session.username != null && par3Session.username.length() > 0) {
			this.skinUrl = "https://crafatar.com/skins/" + mod_SkinFix.getInstance().getUUIDBank().get(par3Session.username) + ".png";
		}
		...
		if (par1StatBase != null) {
			if (par1StatBase.isAchievement()) {
				...
				if (var3.parentAchievement == null
						|| this.mc.statFileWriter.hasAchievementUnlocked(var3.parentAchievement)) {
